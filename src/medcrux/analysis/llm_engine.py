...
def analyze_text_with_deepseek(ocr_text: str) -> dict:
    ...
    **逻辑一致性检查规则**（必须严格执行）：
    - **形状一致性**：BI-RADS 3类要求椭圆形。如果描述中出现"条索状"、"条状"、"不规则形"
      等非椭圆形特征，但结论是3类，**必须**识别为不一致。
    - **边界一致性**：BI-RADS 3类要求边界清晰。如果描述中出现"模糊"、"成角"、"毛刺状"等，
      但结论是3类，**必须**识别为不一致。
    - **回声一致性**：BI-RADS 3类要求均匀低回声。如果描述中出现"不均匀回声"，
      但结论是3类，**必须**识别为不一致。
    - **方位一致性**：BI-RADS 3类要求平行（纵横比<1）。如果描述中出现"纵横比>1"、"不平行"，
      但结论是3类，**必须**识别为不一致。
    - **恶性征象一致性**：如果描述中出现任何恶性征象（毛刺、微钙化、声影、
      条索状低回声+其他可疑特征等），但结论是3类或更低，**必须**发出高风险预警。

    **特别关注**：
    - "条索状低回声"不是椭圆形，不符合BI-RADS 3类定义。如果出现条索状低回声但结论是3类，
      **必须**识别为不一致并发出高风险预警。
    - 条索状低回声 + 导管内占位 + 血流信号 = 高度可疑，不应是3类。
